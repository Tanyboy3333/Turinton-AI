...
def task_qa(act: dict, rng: random.Random) -> list[dict]:
    ...
    for sub in sec.get("subsections", []):
        ...
        if not re.match(rf"^\(?{sub_num}\)?", sub_text):
            continue
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Convert Indian Legal Act JSON files to Alpaca JSONL format"
    )
    ...
    parser.add_argument(
        ...
        help="Directory where Alpaca JSONL files will be saved"
    )
